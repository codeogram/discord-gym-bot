...
Adding the first person is bugged because add_me returns an empty dataframe. -- fixed, ish
Adding to an empty Gym_User_List file now switches the order of the column headers and data. May need to rewrite this whole process.
Add functionality for creating a dates file for a user if it doesn't exist.
Line 155 - if not df.empty -- raises error if df is an int (ie -1)
...
def access_db(query_type=None, id=None):
    ...
    if query_type == 'searching': # eg for display_users(), user_specified(), add_visit & last_visit()
        ...
        try:
            filtered_data = df.loc[id]
            print("we found you, so you don't need adding")
            return -1  # return -1 if the user is found - we shouldn't add them if the user is already there
        except:
            print("didn't find you, so we're gonna add you")
            print(f"trying to give an error: {df ** 2}")
...
def add_me(id,username): # adds the user to the db if they aren't there already
    ...
    if not df.empty: # if the df is empty it means no names are currently there. in that circumstance, this try/else block won't catch it properly, and will say the user is already there.
        try:
            # if the user is already there, df will be -1
            df += 1 # if df returned -1, this will work, and hence indicate the user is already there
    ...
    # (else) if the df IS empty, it means the user is not there (nor anyone else), and hence needs to be added
...
async def on_message(message): # coroutine responsible for running commands based on user input
    ...
    # message data
    if user_message[0] == "!": # will only compute split messages if the message is a command
        ...
        # print(f"user_command: {user_command}") # user command extracted from the message content

    else: # else a command isn't being used
        return
